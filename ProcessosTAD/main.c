...
int main()
{
    ...
    if (!lerCSV("processo_043_202409032338.csv", processos, &total))
    {
        return 1;
    }
    ...
    variosAssuntos(processos, total);

    // contador de quantidade de dias em tramitação
    printf("\nDias em tramitacao de cada processo:\n");
    for (int i = 0; i < total; i++)
    {
        int dias = calcularTramitacao(processos[i].data_ajuizamento);
